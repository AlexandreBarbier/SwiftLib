...
public class ToastView : UIView {
    ...
    private class func initToastView() -> ToastView {
        ...
        view.swipeIndicator.autoresizingMask = [.FlexibleLeftMargin, .FlexibleRightMargin]
    ...
    public func panGesture(pan:UIPanGestureRecognizer) -> Void {
        ...
        if (pan.state == UIGestureRecognizerState.Ended) {
            if (pan.view!.frame.origin.y < -(pan.view!.frame.size.height / 3)) {
                ...
                UIView.animateWithDuration(time, delay: 0.0, usingSpringWithDamping: 0.7, initialSpringVelocity: 0.5, options: UIViewAnimationOptions.TransitionNone, animations: { () -> Void in
                        self.alpha = 1.0
    ...
    private func removeFromWindow() -> Void {
        UIView.animateWithDuration(0.2, animations: { () -> Void in
                ...
                if (finish)
                {
                    UIView.animateWithDuration(0.5, delay: 0.0, usingSpringWithDamping: 0.9, initialSpringVelocity: 0.5, options: UIViewAnimationOptions.TransitionNone, animations: { () -> Void in
                        self.transform = CGAffineTransformMakeTranslation(0, -self.frame.size.height)
                        }, completion: { (finish:Bool) -> Void in
                        if (finish)
                        {
    ...
    override public func  didMoveToWindow() {
        if (self.window != nil)
        {
            UIView.animateWithDuration(0.5, delay: 0.0, usingSpringWithDamping: 0.7, initialSpringVelocity: 0.5, options: UIViewAnimationOptions.TransitionNone, animations: { () -> Void in
