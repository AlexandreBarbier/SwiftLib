...
public class SocialTextView : UITextView {
    ...
    private var mentionRegex : NSRegularExpression = NSRegularExpression()
    private var tagRegex : NSRegularExpression = NSRegularExpression()
    ...
    public override init(frame: CGRect, textContainer: NSTextContainer?) {
        ...
        do {
            mentionRegex = try NSRegularExpression(pattern:"@[A-Za-z0-9_]+", options: NSRegularExpressionOptions.CaseInsensitive)
            tagRegex = try NSRegularExpression(pattern:"#[A-Za-z0-9_]+", options: NSRegularExpressionOptions.CaseInsensitive)
        }
        catch {
    ...
    public override var text : String! {
        ...
        set {
            ...
            self.mentions = self.mentionRegex.matchesInString(newValue, options: NSMatchingOptions.ReportCompletion, range: NSRangeFromString(newValue))
            self.tags = self.tagRegex.matchesInString(newValue, options: NSMatchingOptions.ReportCompletion, range: NSRangeFromString(newValue))
